...
amtConfirmBtn.addEventListener("click", () => {
  ...
  if (action === "buy") {
    ...
    let element = InfoSeedInventory.find((e) => e.id === selectedPlantID);
    // console.log(totalPrice);
    // console.log(unitPrice);
    InfoUserData.gold -= totalPrice;
    if (element === undefined) {
      InfoSeedInventory.push({ id: selectedPlantID, amount: currAmount });
    } else {
      element.amount += currAmount;
    }
  } else if (action === "sell") {
    //   sell
    ...
    let available = InfoBag[indexNum].amount;
    if (available < currAmount) {
      alertWindow("not enough inventory!");
      return;
    }
    InfoBag[indexNum].amount -= currAmount;
    InfoUserData.gold += totalPrice;
  }
